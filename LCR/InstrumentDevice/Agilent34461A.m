...
-(NSString*)readValueBaseModeWithinCount:(NSString*)agilentSerial
    ...
    do{
        ...
        if ((strTemp==nil || strTemp.length<1)) {
            ...
            [self SetMessureMode:mode andCommunicateType:communicateType range:arange];
        ...
        case Agilent34461A_MODE_RES_4W:{
            ...
            while ([timer durationMillisecond]<time) {
                ...
                if ((strTemp==nil || strTemp.length<1) &&[timer1 durationMillisecond]>1500) {
                    [self CloseDevice];
                    [self OpenDevice:nil andCommunicateType:communicateType];
                    [self SetMessureMode:mode andCommunicateType:communicateType range:range];
                    NSLog(@"Set Agilent mode again!");
                    strTemp=[self ReadData:5 andCommunicateType:communicateType];
                }
...
//
-(void)SetMessureMode:(enum Agilent34461AMessureMode)mode andCommunicateType:(enum Agilent34461ACommunicateType)communicateType range:(NSString *)range;
{
    switch (mode) {
        case Agilent34461A_MODE_RES_4W:
        {
            ...
            [self WriteLine:[NSString stringWithFormat:@":SENS:FRES:RANG %@",range] andCommunicateType:communicateType]; usleep(10*1000);   // 电阻的范围0-100ohm，可以根据实际情况设定
            //[self WriteLine:@"MEAS:FRES? 100,0.0001"];
            //@":SENS:FRES:RANG 10000 "
            NSLog(@"range========%@",range);
        }
            break;
        ...
        case Agilent34461A_MODE_RES_2W:
        {
            ...
            [self WriteLine:[NSString stringWithFormat:@":SENS:RES:RANG %@",range] andCommunicateType:communicateType]; usleep(10*1000);   // 电阻的范围0-100ohm，可以根据实际情况设定
            //[self WriteLine:@"MEAS:FRES? 100,0.0001"];
            //:SENS:RES:RANG 100
        }
            break;
...
//            [self WriteLine:@":SENS:CURR:DC:RANG 1" andCommunicateType:communicateType];usleep(10*1000);
        ...
        case Agilent34461A_MODE_VOLT_DC:
        {
            ...
            [self WriteLine:@":SENS:VOLT:DC:NPLC 1" andCommunicateType:communicateType]; usleep(10*1000);  //shorter has faster speed
